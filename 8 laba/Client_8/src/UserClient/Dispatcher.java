        ...
        if (!line.isEmpty()) {//проверяем не пустая ли
            ...
            switch (command) {
                case "exit":
                    return "exit";
                case "help":
                    if (utils.login==null) return connection.runCommand(new Server.Container("help_login"));
                    return connection.runCommand(new Server.Container("help"));
                case "info":
                    return  connection.runCommand(new Server.Container("info"));
                /*case "show":
                    return  connection.runCommand(new Server.Container("show"));*/
                case "add":
                {
                    Route newRoute = Commands.inputRoute(0,input);
                    return connection.runCommand(new Server.Container("add",newRoute));
                }
                ...
                case "execute_script":
                    if (t.hasMoreTokens()) {
                        ...
                        if (history.addScript(new_file)) {
                            return utils.RunScript(new_file);
                ...
                case "history":
                    return connection.runCommand(new Server.Container("history"));
                ...
                case "login":
                    if (t.countTokens()>1){
                        ...
                        String result = connection.runCommand(new Server.Container("login", login, password));
                    ...
                    else return "Формат команды: login username password";
                case "register":
                    if (t.countTokens()>1)
                        return connection.runCommand(new Server.Container("register",t.nextToken(), t.nextToken()));
                    else return "Формат команды: register username password";
